...
double xFitWide_l = 0.25;
double xFitWide_h = 10.0;
...
std::vector<std::pair<double, double>> fitAndFineFit_fixshiftBG(TH1D* histogram, const std::string& fitName, const std::string& fitFormula, int paramCount, double hcalfit_l, double hcalfit_h, std::pair<double,double>& fitqual, double pshift, double nshift, std::vector<double> pol2par, const std::string& fitOptions = "RBMQ0");
...
void pseSlices(int kine=4, 
	       ...
	       int pass=2, 
	       int N=4,
	       double sliceCutMin=0.10,
	       double sliceCutMax=0.30,
	       bool fixBG=true,
	       bool bestclus=true, 
	       bool thin=true,
	       bool wide=false,
  ...
  cout << "Total events in 2D hist: " << hdx_vs_bb_ps_e_data->GetEntries() << endl;

  int xbinCutLow = hdx_vs_bb_ps_e_data->GetXaxis()->FindBin(sliceCutMin);
  int xbinCutHigh = hdx_vs_bb_ps_e_data->GetXaxis()->FindBin(sliceCutMax);

  TH1D* cutProjY_bb_ps_e = hdx_vs_bb_ps_e_data->ProjectionY("_py", xbinCutLow, xbinCutHigh);

  int cutEvents = cutProjY_bb_ps_e->Integral();
  int eventsPerSlice = cutEvents / N;

  cout << "Total events examined range: " << hdx_vs_bb_ps_e_data->GetEntries() << endl;

  int eventsAccumulated = 0;
  int currentBin = 1;
  std::vector<int> sliceBoundaries; // Stores the upper boundary of each slice in bin index
  std::vector<double> cutx_bb_ps_e; // Stores the upper boundary of each slice in x
  std::vector<double> exactSliceEvents; //Stores the total number of events in each slice

  // Before starting the loop, ensure you add the first boundaries
  sliceBoundaries.push_back(xbinCutLow);
  cutx_bb_ps_e.push_back(sliceCutMin);

  cout << "Getting slice boundaries for " << hdx_vs_bb_ps_e_data->GetTitle() << " with " << eventsPerSlice << " events per slice over " << N << " slices." << endl;

  double sum_events_on_slice = 0;

  // Start the loop from the first bin within your range of interest
  for (int i = xbinCutLow; i <= xbinCutHigh; ++i) {
    // Accumulate events in this bin
    int eventsInBin = hdx_vs_bb_ps_e_data->ProjectionY("_py", i, i)->GetEntries();
    eventsAccumulated += eventsInBin;

    // Define a new slice if conditions are met
    if (eventsAccumulated >= eventsPerSlice || i == xbinCutHigh) {
      double sliceXUBound = hdx_vs_bb_ps_e_data->GetXaxis()->GetBinLowEdge(i + 1);
      sliceBoundaries.push_back(i);
      cutx_bb_ps_e.push_back(sliceXUBound);
  ...
  if( sum_events_on_slice != cutEvents ){
    ...
    cout << "  total events on slice: " << cutEvents << endl << endl;
  }

  // Project the TH2D onto a TH1D for the specified x-axis range  
  int xbinCutWideLow = hdx_vs_bb_ps_e_data->GetXaxis()->FindBin(xFitWide_l);
  int xbinCutWideHigh = hdx_vs_bb_ps_e_data->GetXaxis()->FindBin(xFitWide_h);

  TH1D* wideProjY_bb_ps_e = hdx_vs_bb_ps_e_data->ProjectionY("_py", xbinCutWideLow, xbinCutWideHigh );

  // Use Integral() to get the total number of events within the specified range
  int totalEvents_bb_ps_e = wideProjY_bb_ps_e->Integral();

  cout << "Total events in wide range: " << totalEvents_bb_ps_e << endl;

  std::pair<double,double> wideQualp2;

  auto widep2par_vector = fitAndFineFit(wideProjY_bb_ps_e, "sliceFitWide", "fitFullShift_p2", 7, hcalfit_l, hcalfit_h, wideQualp2, fitopt.c_str());

  double fixed_pshift = widep2par_vector[2].first;
  double fixed_nshift = widep2par_vector[3].first;
  std::vector<double> fixed_BGpolpar = {
    widep2par_vector[4].first/wideProjY_bb_ps_e->GetMaximum(),
    widep2par_vector[5].first/wideProjY_bb_ps_e->GetMaximum(),
    widep2par_vector[6].first/wideProjY_bb_ps_e->GetMaximum()
  };

  cout << "Fit over full range of PS E yield shifts p:n -> " << fixed_pshift << ":" << fixed_nshift << endl;

  cout << "Looping over PS E slices.." << endl;
  for (int i = 0; i < N; ++i) {

    int binStart = sliceBoundaries[i]; // The lower boundary of the current slice
    int binEnd = (i < N - 1) ? sliceBoundaries[i + 1] - 1 : xbinCutHigh; // The upper boundary
    ...
    if (!hdx_slice || !dx_bb_ps_e_slice) {
        cout << "Error: hdx_slice or dx_bb_ps_e_slice is null at slice " << i << endl;
        continue;
    }
    ...
    if (!dx_bb_ps_e_p_slice || !dx_bb_ps_e_n_slice || !hdx_p || !hdx_n) {
      cout << "Error: MC slice histograms are null at slice " << i << endl;
      continue;
    }

    //Scale the bg up to meet the new maximum
    std::vector<double> scaledBG;
    double slicemaxvalue = hdx_slice->GetMaximum();

    // Print the scaled vector
    for (double val : fixed_BGpolpar) {
      scaledBG.push_back(val*slicemaxvalue);
    }
    ...
    cout << "Entries: data=" << slice_nEntries << ", p=" << slicemcp_nEntries << ", n=" << slicemcn_nEntries << endl;

    //Rsf extraction using second order poly fit
    std::pair<double,double> sliceQualp2;

    std::vector<std::pair<double, double>> slicep2par_vector;

    if(fixBG)
      slicep2par_vector = fitAndFineFit_fixshiftBG(hdx_slice, "sliceFit", "fitFullShift_p2", 7, hcalfit_l, hcalfit_h, sliceQualp2, fixed_pshift, fixed_nshift, scaledBG, fitopt.c_str());
    else
      slicep2par_vector = fitAndFineFit_fixshift(hdx_slice, "sliceFit", "fitFullShift_p2", 7, hcalfit_l, hcalfit_h, sliceQualp2, fixed_pshift, fixed_nshift, fitopt.c_str());

    cout << "Fit complete for slice " << i << endl;
    ...
    cout << endl << endl << "Writing out all dx vs PS E fit pars: " << endl;
    for (int par=0; par<7; ++par){
      ...
      cout << "p" << par << " " << bb_ps_ereports[i].fitParams[par] << " ";
  ...
  for (int i = 0; i < N; ++i) {
    ...
    bb_ps_ereports[i].sliceHistogram->SetLineWidth(2);
    bb_ps_ereports[i].sliceHistogram->SetLineColor(kBlack);
    bb_ps_ereports[i].sliceHistogram->SetTitleSize(50);
    bb_ps_ereports[i].sliceHistogram->GetXaxis()->SetTitleSize(0.06);
    bb_ps_ereports[i].sliceHistogram->GetXaxis()->SetTitleOffset(0.5);  
    bb_ps_ereports[i].sliceHistogram->SetLineColor(kBlack);
    
    //bb_ps_ereports[i].sliceHistogram->SetMarkerStyle(7);
    //bb_ps_ereports[i].sliceHistogram->SetMarkerColor(kBlack);
    ...
    double xval2 = (bb_ps_ereports[i].winHigh + bb_ps_ereports[i].winLow) / 2;
    double xval = (cutx_bb_ps_e[i+1] + cutx_bb_ps_e[i]) / 2;
    
    cout << xval << " " << xval2 << endl;

    double nev = bb_ps_ereports[i].nev;
    ...
    int transparentPink = TColor::GetColorTransparent(kGreen, 0.3);
    hFromTF1_bb_ps_e->SetLineColor(kGreen);
    ...
    legend->AddEntry((TObject*)0, Form("events: %0.0f", bb_ps_ereports[i].nev), "");
    legend->AddEntry((TObject*)0, Form("MC events: %0.0f", MCev), "");

    legend->AddEntry(bb_ps_ereports[i].sliceHistogram, "data", "l");
    legend->AddEntry(hFromTF1_bb_ps_e, "fit (MC + BG)", "f");
...
std::vector<std::pair<double, double>> fitAndFineFit_fixshiftBG(TH1D* histogram, const std::string& fitName, const std::string& fitFormula, int paramCount, double hcalfit_l, double hcalfit_h, std::pair<double,double>& fitqual, double pshift, double nshift, std::vector<double> pol2par, const std::string& fitOptions = "RBMQ0") {
  ...
  // Fix background polynomial parameters
  fit->FixParameter(4, pol2par[0]);
  fit->FixParameter(5, pol2par[1]);
  fit->FixParameter(6, pol2par[2]);
  ...
  fineFit->FixParameter(4, pol2par[0]);
  fineFit->FixParameter(5, pol2par[1]);
  fineFit->FixParameter(6, pol2par[2]);

  cout << "Fixing fineFit shift and BG parameters at " << pshift << ", " << nshift << endl;
