//sseeds 4.22.24: script to vary course cut on e' E/p and extract Rsf. Comparison on dependent ranges determines fine cut.
...
double xrange_min_eoverp = 0.5;
double xrange_max_eoverp = 1.5;
...
void eOverPStability(int kine=9, 
		     int mag=70, 
		     int pass=2, 
		     int N=12,
		     double mean=1.0,
		     double sigma=0.02,
		     double start_sigma=9.0,
		     double max_sigma=11.0,
		     std::string BG="pol2",
		     bool bestclus=true, 
		     bool thin=true,
		     bool wide=false,
  ...
  std::string foutPath = Form("%s/gmn_analysis/eoverpstab_gmn_sbs%d_mag%d_pass%d%s%s%s.root", basePath.c_str(), kine, mag, pass, bestclus_word.c_str(), wide_word.c_str(), effz_word.c_str());
  ...
  TH2D *hdx_vs_eoverp_data = dynamic_cast<TH2D*>(inputFile->Get("hist_bb_etot_over_p"));
  hdx_vs_eoverp_data->GetYaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
  int hdx_vs_eoverp_data_N = hdx_vs_eoverp_data->GetEntries();
  std::string eoverpCuts = hdx_vs_eoverp_data->GetTitle();
  cout << endl << "Opened dx vs E/p with cuts: " << eoverpCuts << endl << endl; 
  ...
  TH2D *hdx_vs_eoverp_n = dynamic_cast<TH2D*>(inputFileMC->Get("hist_bb_etot_over_p_n"));
  hdx_vs_eoverp_n->GetYaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
  int hdx_vs_eoverp_n_N = hdx_vs_eoverp_n->GetEntries();

  TH2D *hdx_vs_eoverp_p = dynamic_cast<TH2D*>(inputFileMC->Get("hist_bb_etot_over_p_p"));
  hdx_vs_eoverp_p->GetYaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
  int hdx_vs_eoverp_p_N = hdx_vs_eoverp_p->GetEntries();
  ...
  // Pad 2: hdx_vs_eoverp_data
  canvas->cd(2);
  hdx_vs_eoverp_data->Draw("COLZ");
  ...
  // Pad 8: hdx_vs_eoverp_n
  canvas->cd(7);
  hdx_vs_eoverp_n->Draw("COLZ");

  // Pad 10: hdx_vs_eoverp_p
  canvas->cd(8);
  hdx_vs_eoverp_p->Draw("COLZ");
  ...
  std::vector<ReportData> eoverpreports;
  eoverpreports.resize(N);

  // Loop over dx vs E/p
  hdx_vs_eoverp_data->GetXaxis()->SetRangeUser(xrange_min_eoverp, xrange_max_eoverp);
  hdx_vs_eoverp_data->GetYaxis()->SetRangeUser(hcalfit_l, hcalfit_h);

  // Get plot limits
  int xbinLow_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(xrange_min_eoverp);
  int xbinHigh_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(xrange_max_eoverp); 

  // Get cut region
  int xbinmean_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(mean);
  double xCutMax = mean + (max_sigma*sigma);
  double xCutMin = mean - (max_sigma*sigma);
  int xbinCutMax_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(xCutMax);
  int xbinCutMin_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(xCutMin);
  double startcut = start_sigma * sigma;
  int xbinCutStart_low_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(mean-startcut);
  int xbinCutStart_high_eoverp = hdx_vs_eoverp_data->GetXaxis()->FindBin(mean+startcut);

  cout << "Bin at starting sigma RHS = " << xbinCutStart_high_eoverp << endl;

  int NbinsRight = xbinCutMax_eoverp - xbinCutStart_high_eoverp;
  
  cout << "Mean bin " << xbinmean_eoverp << " with rightmost bin " << xbinCutMax_eoverp << ".." << endl;
  ...
  int xbinRanges_eoverp = NbinsRight / N;

  cout << "Each added range per cut: " << xbinRanges_eoverp << ".." << endl;

  cout << "Added bins in x per cut: " << 2*xbinRanges_eoverp << " on N = " << N << " cuts." << endl;

  // Project the TH2D onto a TH1D for the specified x-axis range
  TH1D* fullProjY_eoverp = hdx_vs_eoverp_data->ProjectionY("_py", xbinLow_eoverp, xbinHigh_eoverp);
  TH1D* wideProjY_eoverp = hdx_vs_eoverp_data->ProjectionY("_wpy");

  // Use Integral() to get the total number of events within the specified range
  int totalEvents_eoverp = fullProjY_eoverp->Integral();

  std::pair<double,double> wideQualp2;

  auto widep2par_vector = fitAndFineFit(wideProjY_eoverp, "sliceFitWide", "fitFullShift_p2", 7, hcalfit_l, hcalfit_h, wideQualp2, 0, 0, fitopt.c_str());

  double fixed_pshift = widep2par_vector[2].first;
  double fixed_nshift = widep2par_vector[3].first;

  cout << "Fit over full E/p range yield shifts p:n -> " << fixed_pshift << ":" << fixed_nshift << endl;

  vector<double> cutx_low_eoverp;
  vector<double> cutx_high_eoverp;

  cout << "Looping over E/p ranges.." << endl;
  for (int i = 0; i < N; ++i) {

    // int binStart = xbinmean_eoverp - (i+1)*xbinRanges_eoverp;
    // int binEnd = xbinmean_eoverp + (i+1)*xbinRanges_eoverp;

    int binStart = xbinCutStart_low_eoverp - (i+1)*xbinRanges_eoverp;
    int binEnd = xbinCutStart_high_eoverp + (i+1)*xbinRanges_eoverp;

    if(binStart<xbinLow_eoverp || binEnd>xbinHigh_eoverp){
      cout << "ERROR: cut ranges exceed length of TH2D in x." << endl;
      return;

    }

    cutx_low_eoverp.push_back(hdx_vs_eoverp_data->GetXaxis()->GetBinLowEdge(binStart));
    cutx_high_eoverp.push_back(hdx_vs_eoverp_data->GetXaxis()->GetBinLowEdge(binEnd+1));

    cout << "Cut range from x=" << hdx_vs_eoverp_data->GetXaxis()->GetBinLowEdge(binStart) << " to " << hdx_vs_eoverp_data->GetXaxis()->GetBinLowEdge(binEnd+1) << " (bin " << binStart << " to bin " << binEnd << ")" << endl;

    //Get the data slices
    TH1D *hdx_slice = hdx_vs_eoverp_data->ProjectionY(Form("hdx_eoverp_slice_%d", i+1), binStart, binEnd);
    TH1D *dx_eoverp_slice = hdx_vs_eoverp_data->ProjectionY(Form("dxfidcut_%d", i+1), binStart, binEnd);

    //Get the MC slices
    TH1D *dx_eoverp_p_slice = hdx_vs_eoverp_p->ProjectionY(Form("dxfidcut_p_%d", i+1), binStart, binEnd);
    TH1D *dx_eoverp_n_slice = hdx_vs_eoverp_n->ProjectionY(Form("dxfidcut_n_%d", i+1), binStart, binEnd);
    hdx_p = hdx_vs_eoverp_p->ProjectionY(Form("hdx_eoverp_slice_p_%d", i+1), binStart, binEnd);
    hdx_n = hdx_vs_eoverp_n->ProjectionY(Form("hdx_eoverp_slice_n_%d", i+1), binStart, binEnd);

    //Get total entries on slice
    int slice_nEntries = dx_eoverp_slice->Integral();
    int slicemcp_nEntries = dx_eoverp_p_slice->Integral();
    int slicemcn_nEntries = dx_eoverp_n_slice->Integral();
    ...
    eoverpreports[i] = ReportData( dx_eoverp_slice, 
				 dx_eoverp_p_slice,
				 dx_eoverp_n_slice,
				 cutx_low_eoverp[i],
				 cutx_high_eoverp[i],
    ...
    for (int par=0; par<7; ++par){
      eoverpreports[i].fitParams[par] = slicep2par_vector[par].first;
      eoverpreports[i].fitErrors[par] = slicep2par_vector[par].second;
  ...
  TH2D* clonedEoverp = (TH2D*)hdx_vs_eoverp_data->Clone("clonedEoverp");
  double eoverp_ymin = hcalfit_l;
  double eoverp_ymax = hcalfit_h;

  TCanvas* c0 = new TCanvas("c0", "Slice Locations", 800,600);
  c0->cd();
  clonedEoverp->SetTitle("e' E/p vs dx");
  clonedEoverp->Draw("colz");

  int numberOfLines = cutx_low_eoverp.size(); // Assuming cutx_low_eoverp and cutx_high_eoverp are same size
  ...
  for (int i = 0; i < numberOfLines; ++i) {
    TLine* line = new TLine(cutx_low_eoverp[i], eoverp_ymin, cutx_low_eoverp[i], eoverp_ymax);
  ...
  for (int i = 0; i < numberOfLines; ++i) {
    TLine* line = new TLine(cutx_high_eoverp[i], eoverp_ymin, cutx_high_eoverp[i], eoverp_ymax);
    line->SetLineColor(colorGradient[i]); // Set line color dynamically
    line->SetLineWidth(2); // Set line thickness dynamically
    line->Draw();
    ++lineThickness; // Increase line thickness for next line
  ...
  // for (auto& cut : cutx_low_eoverp){
  //   TLine* line = new TLine(cut, eoverp_ymin, cut, eoverp_ymax);
  //   line->SetLineColor(kRed); // Set line color to red
  //   line->Draw();
  // }

  // for (auto& cut : cutx_high_eoverp){
  //   TLine* line = new TLine(cut, eoverp_ymin, cut, eoverp_ymax);
  //   line->SetLineColor(kRed); // Set line color to red
  //   line->Draw();
  // }
  ...
  std::vector<double> Rsf_vec_eoverp;
  std::vector<double> Rsferr_vec_eoverp;
  std::vector<double> xval_vec_eoverp;
  std::vector<double> sig_vec_eoverp;
  std::vector<double> nev_vec_eoverp;

  TCanvas* canvasSlices_eoverp = new TCanvas("dx slices over E/p", "dx slices over E/p", 1800, 1200);
  
  // Assuming N is the total number of histograms to be plotted on the canvas
  int optimalRows_eoverp = 1;
  int optimalCols_eoverp = 1;
  
  // Find the nearest square root for N to determine the grid size
  int sqrtN_eoverp = (int)std::sqrt(N);
  for (int cols = sqrtN_eoverp; cols <= N; ++cols) {
    if (N % cols == 0) { // If cols is a factor of N
      optimalCols_eoverp = cols;
      optimalRows_eoverp = N / cols;
      break; // Found the optimal layout
    }
  }
  
  canvasSlices_eoverp->Divide(optimalCols_eoverp, optimalRows_eoverp); // Adjust the division based on reportSamples or desired layout
  
  TF1* bgfit_eoverp[N];
  
  // loop over slices
  for (int i = 0; i < N; ++i) {
    if (eoverpreports[i].sliceHistogram == nullptr)
      continue;
    
    canvasSlices_eoverp->cd(i + 1);
    
    //catch bad fit ranges and do not write
    std::string tempTitle = eoverpreports[i].sliceHistogram->GetTitle();      
    if( tempTitle.compare("Null Histogram")==0 )
      continue;
    
    //get mc nucleon parameters
    double p_scale = eoverpreports[i].fitParams[0];
    double n_scale = eoverpreports[i].fitParams[1];
    double p_shift = eoverpreports[i].fitParams[2];
    double n_shift = eoverpreports[i].fitParams[3];
    
    double MCev = eoverpreports[i].mcpnev + eoverpreports[i].mcnnev;
    
    // Simplify the histogram title
    eoverpreports[i].sliceHistogram->SetTitle(Form("%s %0.2f to %0.2f", eoverpreports[0].type.c_str(), eoverpreports[i].winLow, eoverpreports[i].winHigh));
    eoverpreports[i].sliceHistogram->SetLineWidth(1);
    eoverpreports[i].sliceHistogram->Draw();
    
    // Retrieve the number of bins, and the x-axis limits of the slice histogram
    int nbins = eoverpreports[i].sliceHistogram->GetNbinsX();
    double x_low = eoverpreports[i].sliceHistogram->GetXaxis()->GetXmin();
    double x_high = eoverpreports[i].sliceHistogram->GetXaxis()->GetXmax();
    
    //get scale ratio, error, and midpoint for slice
    double ratio = eoverpreports[i].scaleratio;
    double error = eoverpreports[i].scaleratioerr;
    double xval = eoverpreports[i].winLow;
    double sig = (i+1)*((max_sigma-start_sigma)/N)+start_sigma;
    double nev = (totalEvents_eoverp - eoverpreports[i].nev)/totalEvents_eoverp*100;

    //fill vectors for later tgrapherrors
    Rsf_vec_eoverp.push_back(ratio);
    Rsferr_vec_eoverp.push_back(error);
    xval_vec_eoverp.push_back(xval);
    sig_vec_eoverp.push_back(sig);
    nev_vec_eoverp.push_back(nev); //Total remaining stats

    // Draw the corresponding MC histograms
    hdx_p = eoverpreports[i].slicePHistogram; //update the MC slice for the later overall fit
    TH1D *pMCslice = util::shiftHistogramX(eoverpreports[i].slicePHistogram, p_shift);
    pMCslice->Scale(p_scale);
    
    hdx_n = eoverpreports[i].sliceNHistogram; //update the MC slice for the later overall fit
    TH1D *nMCslice = util::shiftHistogramX(eoverpreports[i].sliceNHistogram, n_shift);
    ...
    legend->AddEntry((TObject*)0, Form("Nev: %0.0f", eoverpreports[i].nev), "");
    ...
    legend->AddEntry((TObject*)0, Form("#chi^{2}/ndf: %0.2f", eoverpreports[i].chisqrndf), "");
    legend->Draw();
    
    bgfit_eoverp[i] = new TF1(Form("bgfit_eoverp_%d",i),fits::g_p2fit_cd,hcalfit_l,hcalfit_h,3);
    //cout << "Background fit parameter" << endl;
    for (int j=0; j<3; ++j){
      bgfit_eoverp[i]->SetParameter(j,eoverpreports[i].fitParams[j+4]);
      //cout << "   " << j << " = " << dxreports[i].fitParams[j+4] << endl;
    }
    
    bgfit_eoverp[i]->SetLineWidth(2);
    bgfit_eoverp[i]->Draw("same");
    
    TF1 *mcfit = new TF1(Form("mcfit_%d", i), fitFullShift_p2, hcalfit_l, hcalfit_h, 7);
    mcfit->SetParameters(eoverpreports[i].fitParams);
    ...
    TH1D* hFromTF1_eoverp = new TH1D(Form("hFromTF1_eoverp_%d", i), "Histogram from fid x TF1", nbins, x_low, x_high);
    
    for (int bin = 1; bin <= nbins; ++bin) {
      double binCenter = eoverpreports[i].sliceHistogram->GetBinCenter(bin);
      double funcValue = mcfit->Eval(binCenter);
      hFromTF1_eoverp->SetBinContent(bin, funcValue);
    }
    
    int transparentViolet = TColor::GetColorTransparent(kViolet, 0.3);
    hFromTF1_eoverp->SetLineColor(kViolet);
    hFromTF1_eoverp->SetLineWidth(0);
    hFromTF1_eoverp->SetFillColor(transparentViolet);
    hFromTF1_eoverp->SetFillStyle(1001);
    hFromTF1_eoverp->Draw("SAME LF2");
    
    canvasSlices_eoverp->Update();
  }//endloop over N (slices)
  
  canvasSlices_eoverp->Write();
  
  //Create Graphs
  int numValidPoints_eoverp = Rsf_vec_eoverp.size();

  TGraphErrors* graphErrors_eoverp = new TGraphErrors(numValidPoints_eoverp);
  for (int i = 0; i < numValidPoints_eoverp; ++i) {
    graphErrors_eoverp->SetPoint(i, sig_vec_eoverp[i], Rsf_vec_eoverp[i]);
    graphErrors_eoverp->SetPointError(i, 0, Rsferr_vec_eoverp[i]);
  }
  graphErrors_eoverp->SetMarkerStyle(21);
  graphErrors_eoverp->SetMarkerColor(kViolet);
  graphErrors_eoverp->SetLineColor(kViolet);
  graphErrors_eoverp->SetTitle(Form("R_{sf} vs. N sig (#sigma = %.3f, mean = %.3f); N sig; R_{sf}",sigma, mean));

  TGraph* graph_nev = new TGraph(numValidPoints_eoverp);
  for (int i = 0; i < numValidPoints_eoverp; ++i) {
    graph_nev->SetPoint(i, sig_vec_eoverp[i], nev_vec_eoverp[i]);
  }
  graph_nev->SetMarkerStyle(20);
  graph_nev->SetMarkerColor(kGreen-5);
  graph_nev->SetLineColor(kGreen-5);
  graph_nev->SetTitle(Form("Nev Left in Wide Cut vs. N sig (#sigma = %.3f); N sig; Nev",sigma));
  ...
  graphErrors_eoverp->GetYaxis()->SetLabelSize(0.04);
  graphErrors_eoverp->GetYaxis()->SetLabelFont(42);
  graphErrors_eoverp->GetYaxis()->SetTitleOffset(1.2);
  graphErrors_eoverp->GetYaxis()->SetTitleSize(0.04);
  graphErrors_eoverp->GetYaxis()->SetTitleFont(42); 

  graphErrors_eoverp->Draw("AP");
  ...
  Double_t xmin = graphErrors_eoverp->GetXaxis()->GetXmin();
  Double_t xmax = graphErrors_eoverp->GetXaxis()->GetXmax();
  ...
  leg->AddEntry(graphErrors_eoverp, "R_{sf}", "p");
  leg->AddEntry(graph_nev, "N events", "p");
  leg->AddEntry((TObject*)0, Form("ev tot: %d", totalEvents_eoverp), "");
  leg->Draw();

  std::string displayname = "Cuts on dx vs eoverp (all sigma ranges)";
  util::parseAndDisplayCuts(displayname.c_str(), eoverpCuts.c_str());
  ...
  // TGraphErrors* graphErrors_eoverp = new TGraphErrors(numValidPoints_eoverp);
  // graphErrors_eoverp->SetTitle("Ratio vs E/p cut range; N sig;R_{sf}");
			
  // // Filling the TGraphErrors with data
  // for (int i = 0; i < numValidPoints_eoverp; ++i) {
  //   graphErrors_eoverp->SetPoint(i, sig_vec_eoverp[i], Rsf_vec_eoverp[i]);
  //   graphErrors_eoverp->SetPointError(i, 0, Rsferr_vec_eoverp[i]); // Assuming no error in x
  // }
  ...
  // // Drawing the graph
  // TCanvas* graphCanvas_eoverp = new TCanvas("GraphCanvas_eoverp", "Ratio vs E/p cut range", 1600, 600);
  // graphCanvas_eoverp->cd();
  ...
  // graphErrors_eoverp->Draw("AP"); // Draw with markers and a line connecting points

  // // Save or Write the canvas as needed
  // // graphCanvas->SaveAs(Form("TGraphErrors_Cut_%d.png", r)); // Save to a file
  // graphCanvas_eoverp->Write(); // Or write to an open ROOT file

  // //TCanvas *c1; util::parseAndDisplayCuts(c1,eoverpCuts.c_str());

  // std::string displayname = "Cuts on dx vs eoverp (all ranges)";
  // util::parseAndDisplayCuts(displayname.c_str(), eoverpCuts.c_str());
