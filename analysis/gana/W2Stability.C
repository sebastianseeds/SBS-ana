//sseeds 4.22.24: script to vary course cut on W2 and extract Rsf. Comparison on dependent ranges determines fine cut.
...
//main. kine=kinematic, mag=fieldsetting, pass=pass#, N=total range plots, mean=course cut mean, sigma=course cut std dev, start_sigma=beginning of range slices, lhs_sig_fac=fix number of sigma per slice when llim is fixed, max_sigma=end of range slices, llim_override=fix lower limit on the cut range, BG=background function, bestclus=use plot file with best cluster selection, thin=use plot file without correlations, wide=use plots with wide cuts, effz=use plots with effective z applied, alt=use plot file using MC alt files
