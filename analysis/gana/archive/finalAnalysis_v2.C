...
void finalAnalysis(int kine=4, int mag=50, int pass=2, double sb_min=-2.0, double sb_max=0.7, double shiftX=0.0, double neutronshift=0.0, int N=6, bool detail=true, bool mc_slices=true, bool blind=false, bool bestclus=true, bool thin=true, bool fitrangeoverride=true, bool alt = false) {
  //set draw params
  ...
  struct ReportData {
    ...
    ReportData(TH1D* hist = nullptr, TH1D* histp = nullptr, TH1D* histn = nullptr, double wl = 0, double wh = 0, double nv = 0, double mvp = 0, double mvn = 0, double sr = 0, double cs = 0, std::string t = "")
      : sliceHistogram(hist), slicePHistogram(histp), sliceNHistogram(histn), winLow(wl), winHigh(wh), nev(nv), mcpnev(mvp), mcnnev(mvn), scaleratio(sr), chisqrndf(cs), type(t) {
  ...
  while ((key = dynamic_cast<TKey*>(next()))) {
    ...
    if(iteridx>nregions){
      cout << "ERROR: iteridx > nregions. Adjust reporting parameters for detailed analysis." << endl;
      break;
    }
    ...
    // if(fitrangeoverride){
    //   firstBinWithData = hist->GetXaxis()->FindBin(hcalfit_l);
    //   lastBinWithData = hist->GetXaxis()->FindBin(hcalfit_h);
    // }
	...
	hdx_slice_n = hist_mc_n->ProjectionY(Form("%s_slicemc_n_%d",histName.c_str(), i+1), binLow, binHigh);
      ...
      //TH1D *reportslice_p = hist_mc_p->ProjectionY(Form("reportslice_p_%d_%s", i+1, histName.c_str()), binLow, binHigh);
      //TH1D *reportslice_n = hist_mc_n->ProjectionY(Form("reportslice_n_%d_%s", i+1, histName.c_str()), binLow, binHigh);
      ...
      if(fitrangeoverride){
	//cellslice[i]->GetXaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
	reportslice->GetXaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
	reportslice_p->GetXaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
	reportslice_n->GetXaxis()->SetRangeUser(hcalfit_l,hcalfit_h);
      }
	...
	return;
      ...
      auto slicep2par_vector = fitAndFineFit(cellslice[i], "sliceFit", "fitSlices_p2", 7, hcalfit_l, hcalfit_h, sliceQualp2, fitopt.c_str());
	...
	cout << "On slice N=" << i << " par " << k << "=" << param << " with error " << error << endl;  
	...
	for (int par=0; par<7; ++par){
	  reports[iteridx][currentSampleIndex].fitParams[par] = scalefactor[i][par];
	  reports[iteridx][currentSampleIndex].fitErrors[par] = scaleerr[i][par];
	}
  ...
  for (int r = 0; r < nregions; ++r) {
    ...
    for (int i = 0; i < reportSamples; ++i) {
      ...
      TLegend* legend = new TLegend(0.69, 0.69, 0.89, 0.89); // Adjust the position as needed
      legend->AddEntry((TObject*)0, Form("Nev: %0.0f", reports[r][i].nev), "");
      legend->AddEntry((TObject*)0, Form("MCev: %0.0f", MCev), "");
      legend->AddEntry((TObject*)0, Form("Ratio: %0.2f", reports[r][i].scaleratio), "");
      legend->AddEntry((TObject*)0, Form("#chi^{2}/ndf: %0.2f", reports[r][i].chisqrndf), "");
      legend->Draw();

      // Continue with your fit drawing
      TF1* mcfit = new TF1(Form("mcfit_%d_%d", r, i), fitSlices_p2, hcalfit_l, hcalfit_h, 7);
      mcfit->SetParameters(reports[r][i].fitParams);
      // Assuming SetParErrors method exists and is applicable
      ...
      mcfit->SetFillColor(transparentGreen);
      mcfit->SetFillStyle(1001);
      mcfit->Draw("SAME LF2");
  ...
  // Calculate the total quadrature sum of all contributions
  double totalQuadratureSum = 0.0;
  for (double squaredRelativeError : relativeErrors) {
    totalQuadratureSum += squaredRelativeError;
  }
  double totalSystematicError = sqrt(totalQuadratureSum);

  // Add a line for the total systematic error to the TPaveText
  char totalSystematicErrorText[255];
  sprintf(totalSystematicErrorText, "Estimated Systematic Error: %.4f", totalSystematicError);
