...
double hcalfit_l_or = -1.7; //lower fit/bin limit for hcal dx plots (m) sbs4, 50p
...
double hcalr_l = -1.7; //lower fit/bin limit for hcal dx plots (m) sbs4, 50p
...
void finalAnalysis(int kine=4, int mag=30, int pass=2, double sb_min=-1.6, double sb_max=0.6, double shiftX=0.0, double neutronshift=0.0, int N=12, bool detail=true, bool mc_slices=true, bool blind=false, bool bestclus=true, bool thin=true, bool fitrangeoverride=true, bool alt = false) {
  //set draw params
  ...
  while ((key = dynamic_cast<TKey*>(next()))) {
    ...
    // // Define a secondary palette for the borders
    // const Int_t NCont = 255;
    // const Int_t NRGBs = 5;
    // Double_t stops[NRGBs] = { 0.00, 0.34, 0.61, 0.84, 1.00 };
    // Double_t red[NRGBs] = { 0., 0.25, 0.55, 0.85, 1.00 };
    // Double_t green[NRGBs] = { 0.60, 0.80, 0.90, 0.70, 0.50 };
    // Double_t blue[NRGBs] = { 0.80, 0.70, 0.40, 0.20, 0.00 };
    ...
    for (size_t i = 0; i < sliceBoundaries.size() - 1; ++i) {
      ...
      // box->SetFillColorAlpha(kGray, alpha); // Set color with alpha
      // box->SetFillStyle(1001); // Solid fill
      // box->SetLineColor(kBlack); // Set border color to match the fill color
      // box->SetLineWidth(0); // Set line width to 0 - makes the border invisible
      // box->Draw("same");
	...
	slice_error = 1/sqrt(slice_nEntries);
      ...
      //fix x shift errors from overall fit to data
  ...
  for (int r = 0; r < iteridx; ++r) {
	...
	continue;
      ...
      legend->AddEntry((TObject*)0, Form("Ratio: %0.4f", reports[r][i].scaleratio), "");
