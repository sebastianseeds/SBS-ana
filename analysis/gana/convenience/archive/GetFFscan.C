...
double GNGD_Err[1] = {0};
TString FF_select = "";
double GetFF(const int kID = 4, const double kQ2 = 4.5){/*{{{*/
...
vector<double> Q2_vec;
vector<double> GEn_vec, GMn_vec, GEn_GMn_vec;
...
void GetFFscan(){
    ...
    int Q2_N = (Q2_end - Q2_start)/step_size;

    double Q2_arr[Q2_N], GEn_arr[Q2_N], GMn_arr[Q2_N], GEn_GMn_arr[Q2_N];

    int counter = 0;

    for( double Q2step = 0; Q2step < 10; Q2step += step_size ){
        cout << "Q2: " << Q2step << endl;
        double GEn_val = 0.0, GMn_val = 0.0, GEn_GMn_val = 0.0;
        GEn_val = GetFF(3, Q2step);
        GMn_val = GetFF(4, Q2step);
        GEn_GMn_val = GEn_val/GMn_val;

        Q2_vec.push_back(Q2step);
        Q2_arr[counter] = Q2step;

 // GEp->kID=1, GMp->kID=2, GEn->kID=3, GMn->kID=4
        GEn_vec.push_back(GEn_val );
        GEn_arr[counter] = GEn_val;

        GMn_vec.push_back(GMn_val);
        GMn_arr[counter] = GMn_val;

        GEn_GMn_vec.push_back(GEn_GMn_val);
        GEn_GMn_arr[counter] = GEn_GMn_val;

        counter++;
    }

    TCanvas *c_GEn = new TCanvas("c_GEn", "c_GEn", 600, 500);

    TGraph *tg_GEn = new TGraph(Q2_N, Q2_arr, GEn_arr);
    tg_GEn->Draw();

    TCanvas *c_GMn = new TCanvas("c_GMn", "c_GMn", 600, 500);

    TGraph *tg_GMn = new TGraph(Q2_N, Q2_arr, GMn_arr);
    tg_GMn->Draw();

    TCanvas *c_GEn_GMn = new TCanvas("c_GEn_GMn", "c_GEn_GMn", 600, 500);

    TGraph *tg_GEn_GMn = new TGraph(Q2_N, Q2_arr, GEn_GMn_arr);
    tg_GEn_GMn->Draw();
