...
void pHDEmap(int kine=8, 
  ...
  std::string fout_path = outdir_path + Form("/gmn_analysis/pHDEmap_sbs%d_mag%d_pass%d%s.root",kine,mag,pass,effz_word.c_str());
  ...
  TLegend *lxexp = new TLegend(0.11, 0.6, 0.5, 0.89);
  ...
  TLegend *lyexp = new TLegend(0.11, 0.6, 0.5, 0.89);
  ...
  leg_x->AddEntry(fit_hx, Form("Mean Eff = %.3f #pm %.3f", xMeanEff, fit_hx->GetParError(0)), "L");
  leg_x->Draw();
  ...
  leg_y->AddEntry(fit_hy, Form("Mean Eff = %.3f #pm %.3f", yMeanEff, fit_hy->GetParError(0)), "L");
  leg_y->Draw();
