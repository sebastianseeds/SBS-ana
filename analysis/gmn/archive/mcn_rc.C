//SSeeds 9.1.23 Script to aggregate simc mc neutron output, build a histogram, fit, and extract parameters for further analysis
...
const Double_t ebeam = 5.9826; //GeV, from simc input file
...
void mcn_rc( Int_t kine=8, Int_t mag=70, Int_t cluster_idx=6)
{ //main  
  ...
  std::string fout_path = outdir_path + Form("/gmn_analysis/mcp_rc_out_sbs%d_mag%d_clusteridx%d.root",kine,mag,cluster_idx);
  ...
  std::string rfname = rootfile_dir + Form("/*SIMC_gmn_SBS%d_LD2_%s*.root",kine,nucleon);
  ...
  while (C->GetEntry(nevent++)) {
    ...
    //cout << "On event " << nevent << " atime_sh[0] " << atime_sh[0] << endl;
