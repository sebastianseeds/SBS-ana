//SSeeds 9.1.23 Script to aggregate simc generator mc output by nucleon, build a histogram, fit, and extract parameters for further analysis
...
// const Double_t ebeam_sbs8 = 5.9826; //GeV, from simc input file
// const Double_t ebeam_sbs9 = 5.9826; //GeV, from simc input file
...
//// 2) "intime" algorithm: hcal/bbcal coin time cut then get highest energy cluster left
//// 3) "ET score" algorithm: Obtain gaus fit to coin time and use to compute probability density per event, multiply by cluster energy / max cluster energy for score. Max score wins per event.
void mc_rc( Int_t kine=8, Int_t mag=70, const char *nucleon = "both", Int_t cluster_idx=1)
{ //main  
  ...
  ////////////////////////////////////////////////////////
  //TEMPORARY before MC files run personally
  std::string rootfile_dir = "/lustre19/expphy/volatile/halla/sbs/jboyd/simulation/out_dir/MC_REPLAY_OUT_DIR/simc";
  ////////////////////////////////////////////////////////
  ...
  std::string fout_path = outdir_path + Form("/gmn_analysis/mc_rc_%s_out_sbs%d_mag%d_clusteridx%d.root",nucleon,kine,mag,cluster_idx);
  ...
  if( nucleon.compare("proton")==0 )
    C->Add(rfname_p.c_str());
  else if( nucleon.compare("neutron")==0 )
    C->Add(rfname_n.c_str());
  else if( nucleon.compare("both")==0 ){
    C->Add(rfname_p.c_str());
    C->Add(rfname_n.c_str());
  }else{
  ...
  if( snuc.compare("proton")==0 )
    nuc = "p";
  else if( snuc.compare("neutron")==0 )
    nuc = "n";
  else{
    cerr << "ERROR: nucleon passed to macro must be neutron or proton. Nucleon passed: " << nucleon << endl;
    return;
  }
  ...
  while (C->GetEntry(nevent++)) {
	...
	c_thetapqdiff = cthetapq_p;
      ...
      if( snuc.compare("proton")==0 )
	dxdydist = sqrt( pow(cdx-dx0_p,2)+pow(cdy-dy0,2));
      else
	dxdydist = sqrt( pow(cdx-dx0_n,2)+pow(cdy-dy0,2));
	...
	c_tpq_atimediff = cthetapq_p; 
      ...
      Double_t dxdydist_N;
      if( snuc.compare("proton")==0 )
	dxdydist_N = sqrt( pow(cdx-dx0_p,2)+pow(cdy-dy0,2));
      else
	dxdydist_N = sqrt( pow(cdx-dx0_n,2)+pow(cdy-dy0,2));
