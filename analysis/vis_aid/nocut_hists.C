...
//MAIN
void nocut_hists( Int_t kine=4, Int_t pass=0, Int_t magset=-1 )
{
  ...
  //set up output files
  std::string fout_path = Form("/volatile/halla/sbs/seeds/vis_aid/vis_aid_out_sbs%d.root",kine);
  ...
  //E-arm
  TH1D *hW2_nocut = new TH1D( "hW2_nocut", "W^{2}, no cut; W^{2} (GeV^{2})", 1000, 0.0, 10.0 );
  TH1D *hW2_nocut_ekine = new TH1D( "hW2_nocut_ekine", "W^{2}, no cut, from tree; W^{2} (GeV^{2})", 1000, 0.0, 10.0 );
  ...
  for (Int_t irun=0; irun<nruns; irun++) {
    ...
    cout << mag << " " << runnum << " " << tar << endl;

    if( tar.compare("LD2")!=0 ) 
      continue;
