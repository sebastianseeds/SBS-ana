...
//______________________________________________________________________________
bool JSONManager::DoesSubKeyExist(std::string keyName, std::string subKeyName) const{
  auto it_key = fObject[keyName].find(subKeyName);  // this is an iterator 
  if (it_key!=fObject[keyName].end() ){
    return true;  // not at the end of fObject -- found the key 
  }else{
    std::cout << "[JSONManager::DoesKeyExist]: Key '" << subKeyName << "' does not exist!" << std::endl;
    return false;  // at the end of fObject -- didn't find the key 
  }
}
//______________________________________________________________________________
...
std::string JSONManager::GetValueFromSubKey_str(std::string key,std::string subKey) const{
  bool exist = DoesKeyExist(key);
  if(exist){
    bool subexist = DoesSubKeyExist(key,subKey);
    if(subexist){
...
//______________________________________________________________________________
int JSONManager::GetVectorFromSubKey_str(std::string key,std::string subKey,std::vector<std::string> &data){
  int N=0;
  bool exist = DoesKeyExist(key);
  if(exist){
    bool subexist = DoesSubKeyExist(key,subKey);
    if(subexist){
      ...
      for(int i=0;i<N;i++){
	data.push_back(fObject[key][subKey][i]);
      }
    }else{
      return 1;
    }
