...
class JSONManager {
 ...
 public:
  ...
  bool DoesSubKeyExist(std::string keyName,std::string subKeyName) const;

  int GetVectorFromKey_str(std::string key,std::vector<std::string> &data);
  int GetVectorFromSubKey_str(std::string key,std::string subKey,std::vector<std::string> &data);
  ...
  template <typename T> 
	...
	if( fObject[key][subKey].is_string() ){
      ...
      }else{ // key doesn't exist, return 0
	val = (T)(0);
      }
    }else{
      // key doesn't exist, return 0
